...
func TestMaxGECOSFields(t *testing.T) {
	users, err := ReadDBFromFile(2001, 2001, 2001, 2001, mockDBFilename)
	assert.Nilf(t, err, "Unexpected error: '%s'", err)
	assert.Equalf(t, 1, len(users), "Only one user is expected, got: %v", users)
	expected := GECOS{
		FullName:  "John Doe",
		...
		WorkPhone: "foo",
		HomePhone: "bar",
		Raw:       "John Doe,foobar,foo,bar,bar,foo",
	}
	assert.EqualValues(t, expected, users[0].GECOS)
}
